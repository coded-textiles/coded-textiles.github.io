...
function draw() {
    ...
    intergroupConverge(blooms_0);
    intragroupDiverge(blooms_0, blooms_1);
    intergroupConverge(blooms_1);
    intragroupDiverge(blooms_1, blooms_0);
}


// Inter-group converge
function intergroupConverge(my_group) {
    for (let j = 0; j < n_blooms; j++) {
        let closest_dist = 9000;
        let closest_neighbor_index = (j + 1) % n_blooms;
        let closest_neighbor = my_group[closest_neighbor_index];

        for (let i = 0; i < n_blooms; i++) {
            let this_bloom = my_group[j];
            if (i !== j) {
                let test_neighbor_index = (i + 1) % n_blooms;
                let test_neighbor = my_group[test_neighbor_index];
                let test_dist = dist(
