...
// Bezier anchor
var bezierAnchor = squareWidth / 2;
...
function connect(ax, ay, bx, by) {
	bezier(ax, ay, bezierAnchor, bezierAnchor, bezierAnchor, bezierAnchor, bx, by);
}

function drawQuad0(hasHighlight) {
	// drawQuadBorder();
	connect(corners[0][0], corners[0][1], edges[0][0], edges[0][1]); // Line A
	connect(corners[1][0], corners[1][1], edges[1][0], edges[1][1]); // Line B
	if (hasHighlight) {
		turnHighlightOn();
	}
	connect(corners[2][0], corners[2][1], edges[2][0], edges[2][1]); // Line C
	turnHighlightOff();
	connect(corners[3][0], corners[3][1], edges[3][0], edges[3][1]); // Line D
}

function drawQuad1(hasHighlight) {
	// drawQuadBorder();
	connect(corners[0][0], corners[0][1], edges[0][1], edges[0][0]); // Line A
	connect(corners[1][0], corners[1][1], edges[3][1], edges[3][0]); // Line B
	if (hasHighlight) {
		turnHighlightOn();
	}
	connect(corners[2][0], corners[2][1], edges[2][1], edges[2][0]); // Line C
	turnHighlightOff();
	connect(corners[3][0], corners[3][1], edges[1][1], edges[1][0]); // Line D
}
