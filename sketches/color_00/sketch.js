...
function setup() {
	...
	numGenSlider = createSlider(0, 100, 24, 1);
	numGenText.parent('controls-container');
	numGenSlider.parent('controls-container');
	numGenSlider.class('slider');

	// Num generations
	const mutationRateText = createP("Mutation rate (none - likely)");
	mutationRateSlider = createSlider(0, 2, .2, .01);
...
class Triangle {
	...
	mutateByChance() {
		let mutationMultiplier = mutationRateSlider.value(); // btwn 0 and 1
		let rx = random();
		if (rx < .05 * mutationMultiplier) {
			this.x1 += unitWidth; // right full
			this.x2 += unitWidth; // right full
			this.x3 += unitWidth; // right full
		} else if (rx < .1 * mutationMultiplier) {
			this.x1 -= unitWidth; // left full
			this.x2 -= unitWidth; // left full
			this.x3 -= unitWidth; // left full
		} else if (rx < .15 * mutationMultiplier) {
			this.x1 += unitWidth / 2; // right half
			this.x2 += unitWidth / 2; // right half
			this.x3 += unitWidth / 2; // right half
		} else if (rx < .2 * mutationMultiplier) {
			this.x1 -= unitWidth; // left half
			this.x2 -= unitWidth; // left half
			this.x3 -= unitWidth; // left half
		}

		let ry = random();
		if (ry < .05 * mutationMultiplier) {
			this.y1 += unitHeight; // down full
			this.y2 += unitHeight; // down full
			this.y3 += unitHeight; // down full
		} else if (ry < .1 * mutationMultiplier) {
			this.y1 -= unitHeight; // up full
			this.y2 -= unitHeight; // up full
			this.y3 -= unitHeight; // up full
		} else if (ry < .15 * mutationMultiplier) {
			this.y1 += unitHeight / 2; // down half
			this.y2 += unitHeight / 2; // down half
			this.y3 += unitHeight / 2; // down half
		} else if (ry < .2 * mutationMultiplier) {
			this.y1 -= unitHeight / 2; // up half
			this.y2 -= unitHeight / 2; // up half
			this.y3 -= unitHeight / 2; // up half
		}
	}
...
function drawChild(_P1, _P2) {
	...
	for (let i = 0; i < thisSquare.Triangles.length; i++) {
		...
		thisSquare.Triangles[i].mutateByChance();
		// mutateByChance();
