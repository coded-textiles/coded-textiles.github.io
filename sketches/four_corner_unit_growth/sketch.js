...
function setup() {
	...
	for (i = 0; i < 5; i++) {
		...
		console.log(randomFloat[i]);
	}

	// Generate some random binary decisions to be used
	// for deciding between a curve or line later
	for (i = 0; i < 5; i++) {
		randomZeroOrOne[i] = Math.round(random(0, 1));
		console.log(randomZeroOrOne[i]);
	}
...
function drawConnector(startX, startY, endX, endY, bezierAnchorX, bezierAnchorY, randomZeroOrOne) {
	if (randomZeroOrOne == 0) {
		drawLine(startX, startY, endX, endY);
	} else {
		drawBezier(startX, startY, endX, endY, bezierAnchorX, bezierAnchorY);
	}
}
...
function drawBlock0() {
 	if (cornerCounter == 0) {
		...
		drawConnector(startX, startY, endPointA[0], endPointA[1], bezierX, bezierY, randomZeroOrOne[0]);
		cornerCounter++;
		...
		drawConnector(startX, startY, endPointB[0], endPointB[1], bezierX, bezierY, randomZeroOrOne[1]);
		cornerCounter++;
		...
		drawConnector(startX, startY, endPointC[0], endPointC[1], bezierX, bezierY, randomZeroOrOne[2]);
		cornerCounter++;
		...
		endPointD[1] = 0;
		drawConnector(startX, startY, endPointD[0], endPointD[1], bezierX, bezierY, randomZeroOrOne[3]);
